...
const Attribute = ({ entries = [], info = {} }) => {
    ...
    const exportToExcel = () => {
        ...
        if (!bran || bran.length === 0) {
            setMessage({ id: Date.now(), mgs: 'No attribute to export!' });
            return;
        }
        let excel = [];
        bran.map((item) => {
    ...
    const GetAttribute = async () => {
        // setIsLoading(true)
        const token = localStorage.getItem('token')
        const response = await fetch(`${BaseUrl}/api/get/attribute/tree`, {
            method: 'GET',
            headers: {
                "authorization": token,
                'Content-type': 'application/json; charset=UTF-8',
                        ...
                        <div ref={targetRef} className="pt-3 w-full overflow-hidden overflow-x-auto">
                            <Pdf>
                                <table class="min-w-[600px] w-full text-sm text-left rtl:text-right text-gray-500 mt-4">
                                    ...
                                    <tbody>
                                        {bran?.map((item, i) => (<AttributeCardPdf item={item} i={i} />))}
                                    </tbody>
                                </table>
                            </Pdf>
                        </div>
