  ...
  return (
    <div className='w-full flex justify-start items-center gap-1.5'>
      ...
      <select id={11111} value={selectedId} onChange={handleSelect}
        className={`bg-gray-50 dark:bg-[#040404] border font-thin border-gray-300 text-gray-900 dark:text-white text-sm focus:ring-blue-500 focus:outline-none focus:border-blue-500 block py-1 px-3 ${className}`}
      >
        {options?.map(({ id, name }) => (
          <option key={id} value={id}> {name}</option>
        ))}
      </select>
      <h1 className="mb-2 text-start text-sm text-gray-900 font-normal pt-2 dark:text-white">entries</h1>
    </div>
  );
};
