...
const SellCard = ({ i, item, changeqty, changeprice, changedis, onClick, ChangeDiscountType }) => {
    ...
    return (
        <tr className={` ${i % 2 === 0 ? ' bg-gray-100' : ''} border-b border-x text-[15px] text-black`}>
            ...
            <th scope="col" className="pl-2 py-2 font-thin text-center border-l">{item?.id}</th>
            <th scope="col" className="px-2 py-2 text-left font-thin border-l">{item?.name}</th>
            <th scope="col" className="text-left font-thin border-l">
                <input placeholder={item?.qty} value={item?.qty} onChange={(e) => { changeqty(item?.id, e.target.value) }} className='w-full h-full py-2 focus:outline-none pl-2' />
            </th>
            <th scope="col" className="py-2 text-center font-thin border-l">{item?.price}</th>
            <th scope="col" className="border-l text-left font-thin">
