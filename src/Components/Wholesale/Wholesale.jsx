            ...
            <div className='bg-[#FFFFFF] rounded-md'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4'>
                    <div className='flex justify-start items-end pb-1 z-40'>
                        <SelectionComponent default_select={first} options={state} onSelect={(v) => { setSecond(true); setFirst(false); setCustomer([]); GetCustomer(v?.id) }} label={"Thana Name"} className='rounded-l z-50' />
                    ...
                    <div className='flex justify-start items-end pb-1 z-30'>
                        <SelectionComponent default_select={second} options={customer} onSelect={(v) => { setSecond(false); setFirst(false); setEdition(true); setUserId(v.id); setName(v?.name); fetchUserDue(v.id) }} label={"Customer"} className='rounded-l' />
                ...
                <div className='p-4 w-full overflow-hidden overflow-x-auto'>
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                        ...
                        <tbody>
                            {allData?.map((item, i) => {
                                return <WholeSaleCard key={i} inputQty={inputQty} item={item} changeqty={ChangeQty} changedis={ChangeDiscount} ChangeDiscountType={ChangeDiscountType} changeprice={ChangePrice} onClick={HandleDelete} />
                            })}
                        </tbody>
                    </table>
                </div>
...
//     const [cateHide, setCateHide] = useState(false);
//     const [bandHide, setBandHide] = useState(false);
//     const [itemHide, setItemHide] = useState(false);

//     const [edition, setEdition] = useState(editio)
//     const [editionSelect, setEditionSelection] = useState({ id: 1, name: '' })
//     const [categorySelect, setCategorySelection] = useState({ id: 1, name: '' })
//     const [brandSelect, setBrandSelection] = useState({ id: 1, name: '' })
//     const [itemSelect, setItemSelection] = useState({ id: 1, name: '' })
//     const editi = useRef(null)
//     const cate = useRef(null)
//     const bran = useRef(null)
//     const ite = useRef(null)
//     const disc = useRef(null)
//     const [tempItem, setTempitem] = useState({ price: 0 })
...
//     const [secenosearchItem, setSecondSearchItem] = useState('')
//     const SecondSearchProduct = async (e) => {
//         setSelectedId(0)
//         const name = e.target.value
//         setSecondSearchItem(name)
//         const token = localStorage.getItem('token')
//         if (name) {
//             const response = await fetch(`${BaseUrl}/api/get/product/search/with/${editionSelect?.id}/${categorySelect?.id}/${brandSelect?.id}/${name}`, {
//                 method: 'GET',
//                 headers: {
//                     'authorization': token,
//                 },
//             });
//             const data = await response.json();
//             setSecondSearchData(data.items)
//         } else {
//             setSecondSearchData([]);
//         }
//     }
...
//     const PushItem = (item, qty) => {
//         const newItem = { ...item, qty: qty ? qty : 1 }
...
//     const handleEditionFilter = (e) => {
//         setSelect(e.target.value)
//         const searchValue = e.target.value.toLowerCase();
//         const filterData = edition.filter(item =>
//             item?.name?.toLowerCase().includes(searchValue)
//         );
//         setEdition(filterData);
//     };
...
//         setSelect(e.target.value)
//         const searchValue = e.target.value.toLowerCase();
//         const filterData = brand.filter(item =>
//             item?.name?.toLowerCase().includes(searchValue)
//         );
//         setEdition(filterData);
//     };
...
//                     <div className='relative'>
//                         <Calendar label={"Date"} value={handleDateConvert(new Date(raw?.fromDate))} getDate={(date) => { setValues({ ...values, from_date: date }) }} getTime={(ti) => { setRaw({ ...raw, fromDate: ti }) }} />
//                     </div>


//                     <div className='flex justify-start items-end pb-1 z-30'>
//                         <SelectionComponent default_select={second} options={customer} onSelect={(v) => { setSecond(false); setFirst(false); inputRef.current.focus(); setUserId(v.id); setName(v?.name); fetchUserDue(v.id); setMainSearch(true) }} label={"Customer"} className='rounded-l' />
//                         <div onClick={() => { goto('/create/customer') }} className='border-y border-r px-3 pt-[7px] pb-[6px] rounded-r cursor-pointer text-[#3C96EE] '>
//                             <Add />
//                         </div>
//                     </div>
//                     <div></div>
//                     <Calendar label={"Delivery Date"} value={handleDateConvert(new Date(raw?.toDate))} getDate={(date) => { setValues({ ...values, deliverydate: date }) }} getTime={(ti) => { setRaw({ ...raw, toDate: ti }) }} />
...
//                 <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3 p-4 '>
//                     <div>
//                         <SelectionComponent options={edition} default_select={second} onSelect={() => { }} label={'Edition'} />
//                     </div>
//                     <div>
//                         <SelectionComponent options={category} default_select={second} onSelect={() => { }} label={'Category'} />
//                     </div>
//                     <div>
//                         <SelectionComponent options={brand} default_select={second} onSelect={() => { }} label={'Brand'} />
//                     </div>
//                     <div className='grid col-span-3'>
//                         <h1 className='pb-1'>Enter Item Name</h1>
//                         <div className='flex justify-center w-full h-[39px]'>
...
//                                 <BarCode className='text-[#3C96EE]' />
//                             </div>
//                             <div className='relative border-y text-black w-full z-50'>
//                                 <input type='text' ref={inputRef} placeholder={'Scan Barcode/Search Items'} value={searchItem}
//                                     onClick={() => { setMainSearch(true) }}
//                                     onChange={SearchProduct} className='p-1 mt-[2px] rounded focus:outline-none w-full font-thin'
//                                     onKeyDown={(e) => {
//                                         if (e.key === "ArrowDown") {
//                                             if (searchData?.length === 0 && allData?.length > 0) {
//                                                 inputQty.current.focus()
...
//                                                 // setSelectedId(0)
//                                             } else {
//                                                 PushItem(searchData[selectedId])
//                                                 const newItem = { ...searchData[selectedId], qty: 1 };
//                                                 setAllData([...allData, newItem]);
//                                                 setSearchData([]);
//                                                 setSearchItem('');
//                                                 setSelectedId(0)
...
//                                 />
//                                 <Search className='absolute right-1 top-2 cursor-pointer hover:bg-slate-200 rounded-full' />
//                                 {mainSearch && searchData && searchData?.length > 0 && <div className='w-full absolute top-[35px] border bg-[#FFFFFF] shadow rounded-b'>
//                                     <table className="w-full text-sm text-left rtl:text-right text-gray-500 z-50 bg-white">
//                                         <thead className="text-xs text-gray-900">
//                                             <SearchResultHeader />
//                                         </thead>
//                                         <tbody>
//                                             {searchData?.map((item, i) => {
//                                                 return <tr key={i} className={`border-b cursor-pointer z-50 ${selectedId === i ? 'bg-gray-100' : ''}`} onClick={() => { PushItem(item) }}>
//                                                     <th scope="col" className="px-1 py-2 font-thin text-left">{item?.name}</th>
//                                                     <th scope="col" className="px-1 py-2 font-thin text-left">{item?.edition}</th>
//                                                     <th scope="col" className="px-4 py-2 text-left font-thin">{item?.brand?.name}</th>
//                                                     <th scope="col" className="px-4 py-2 text-left font-thin">{item?.category?.name}</th>
