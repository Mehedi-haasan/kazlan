...
const Order = ({ info = {}, shop = [] }) => {
    ...
    const exportToExcel = () => {
        let filename = 'allorder.xlsx'
        if (!data || data.length === 0) {
            setMessage({ id: Date.now(), mgs: 'No order to export!' });
            return;
        }
        let excel = [];
        data.map((item) => {
            ...
            <div className="rounded-xl overflow-hidden p-4 bg-[#FFFFFF] dark:bg-[#040404] dark:text-white shadow-lg mt-4 pb-20">
                ...
                <div className='flex justify-between items-center my-3'>
                    ...
                    <div className="flex justify-end items-center gap-8">
                        <Excel filename='allinvoice.xlsx' data={data} onClick={() => setPreview(true)} Jpg={() => setPreview(true)} />
