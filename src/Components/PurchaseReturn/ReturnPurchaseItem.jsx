...
const ReturnPurchaseItem = ({shop=[], info = {} }) => {
    ...
    const exportToExcel = () => {
        let filename = 'purchasereturnitems.xlsx'
        if (!invoices || invoices.length === 0) {
            setMessage({ id: Date.now(), mgs: 'No return items to export!' });
            return;
        }
        let excel = [];
        invoices.map((item) => {
            ...
            <div className='rounded-xl overflow-hidden p-4 bg-[#FFFFFF] dark:bg-[#040404] dark:text-white shadow-lg mt-4 pb-20'>
                ...
                <div className="flex justify-between items-center ">
                    ...
                    <div className="flex justify-end items-center gap-8">
                        <Excel filename='purchaseretuenitems.xlsx' data={invoices} onClick={() => setPreview(true)} Jpg={() => setPreview(true)} is_delete={true} />
                        <Search SearchProduct={(v) => SearchOrder(v)} />
                    </div>
                </div>
                <InvoiceTemp invoices={invoices} usertype="Supplier" date_type={"Return Date"} is_due={false} />
                <Modal show={preview} handleClose={() => { setPreview(false) }} size={`1100px`} crosshidden={true}>
                    <div ref={ref} className="px-3 py-4">
                        <div ref={targetRef} className="pt-3 w-full overflow-hidden overflow-x-auto actual-receipt max-h-[80vh] overflow-y-auto" >
                            {/* <Pdf> */}
                            <h1 className="text-center">Product Purchase Return Details</h1>
                            <PdfInvoiceTemp invoices={invoices} info={info} usertype="Supplier" date_type={"Return Date"} is_due={false} />
                            {/* </Pdf> */}
                        </div>
                    </div>
