...
const Edition = ({ options, onSelect, onChange, label, className = 'rounded', default_select = false }) => {

  const inputRef = useRef(null);
  const outside = useRef(null);
  const [selectedId, setSelectedId] = useState(0);
  const [select, setSelect] = useState('Select a edition');
  ...
  return (
    <div className='w-full bg-white'>
      ...
      <div ref={outside} className={`${hide ? 'border-t border-x pr-[1px]' : 'border '} relative bg-white h-[39px] z-10`}>
