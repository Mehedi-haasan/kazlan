...
const PurchaseItems = ({ info = {} }) => {
    ...
    const exportToExcel = () => {
        let filename = 'purchaseitems.xlsx'
        if (!invoices || invoices.length === 0) {
            setMessage({ id: Date.now(), mgs: 'No purchase items to export!' });
            return;
        }
        let excel = [];
        invoices.map((item) => {
    ...
    return (
        <div className="px-3 pt-5 rounded-md min-h-screen">
            ...
            <div className='flex justify-between items-center p-4 bg-[#FFFFFF] dark:bg-[#040404] dark:text-white shadow-md rounded-lg'>
                <h1 className=''>Recent Purchase</h1>
                <NavLink to={`/purchase/product`} className={`border rounded-md shadow bg-blue-500 text-white py-1.5 px-4 font-thin`}>Create Purchase</NavLink>
            </div>
            ...
            <div className='rounded-xl overflow-hidden p-4 bg-[#FFFFFF] dark:bg-[#040404] dark:text-white shadow-lg mt-4 pb-20'>
                ...
                <InvoiceTemp invoices={invoices} usertype="Supplier" date_type={"Receive Date"} />
