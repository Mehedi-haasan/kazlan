...
const SaleReturn = ({ shop = [], editio = [], brand = [], category = [], state = [], info = {} }) => {
    ...
    const Order = async () => {
        if (!userId) {
            setMessage({ id: Date.now(), mgs: "Customer are required" });
            return
        }
        const token = localStorage.getItem('token');
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-10 gap-3 p-4 '>
                    ...
                    <div className='grid col-span-6'>
                        ...
                        <div className='flex justify-center w-full h-[39px] relative'>
                            ...
                            {searchData && searchData?.length > 0 && <div className='w-full absolute top-[36px] border bg-[#FFFFFF] dark:bg-[#040404] dark:text-white shadow rounded-b'>
                        ...
                        <div>
                            {Object.keys(prepareData || {}).length > 0 && (
                                <div className={`border-b border-x text-[15px] text-black grid grid-cols-12`}>
                                    ...
                                    <div className="px-1 py-1 text-left font-thin border-l grid col-span-2">{prepareData?.category?.name}</div>
                                    <div className="px-1 py-1 text-left font-thin border-l grid col-span-2">{prepareData?.brand?.name}</div>
